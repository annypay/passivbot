...
impl<'a> Backtest<'a> {
        ...
        let symbols: Vec<orchestrator::SymbolInput> = indices
            ...
            .map(|idx| {
                ...
                orchestrator::SymbolInput {
                    ...
                    emas,
                    forager_m1: None,
                    // The decision timestamp for this bar. `get_orchestrator_input_cached`
                    // refreshes it for every symbol on every bar, and a freshly built
                    // input has to agree with the cached one.
                    regime_eval_ts_ms: Some(
    ...
    fn get_orchestrator_input_cached(
        ...
        for sym in input.symbols.iter_mut() {
            ...
            // Entry-regime gate: hand the bar's timestamp to the orchestrator, which
