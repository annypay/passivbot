...
def build_matrix(
    ...
    *,
    ...
    `declared_twe` is the arm's configured total wallet exposure limit; it replaces the
    hard-coded 1.0 bound so the same matrix works for the published profile and for a
    deliberately de-levered or levered arm. The liquidation inputs (starting balance,
    threshold and the balance at the peak) let the matrix state the adverse move that would
    end the run at the engine's floor.
    """
    ...
    return WipeoutMatrix(
        ...
        declared_twe=None if declared_twe is None else float(declared_twe),
        starting_balance_usd=starting_balance_usd,
        liquidation_threshold=liquidation_threshold,
        liquidation_shock=shock,
    )
...
def liquidation_shock(
    *, peak_exposure: float, balance_usd: float, starting_balance: float, threshold: float
    ...
    At exposure `e` (position value / balance) a move `r` costs `e * |r| * balance`; the run
    ends when equity reaches `starting_balance * threshold`. Returns `None` when there is
    nothing to lose or the account is already at/below the floor.
    """
    if peak_exposure <= 0.0 or not math.isfinite(balance_usd) or balance_usd <= 0.0:
        return None
    room = float(balance_usd) - float(starting_balance) * float(threshold)
    if room <= 0.0:
        return None
    return room / (float(peak_exposure) * float(balance_usd))
...
def assert_identities(matrix: WipeoutMatrix) -> list[str]:
    ...
    problems: list[str] = []
    bound = matrix.declared_twe
    if bound is not None and matrix.peak_total_exposure > bound + 1e-3:
        problems.append(
            f"{matrix.arm}: total_wallet_exposure_max {matrix.peak_total_exposure} exceeds the "
            f"declared total_wallet_exposure_limit {bound}"
        )
    if matrix.declared_twe is not None and matrix.declared_twe <= 0.0:
        problems.append(f"{matrix.arm}: declared total wallet exposure limit must be positive")
    if matrix.liquidation_shock is not None and not (
        math.isfinite(matrix.liquidation_shock) and matrix.liquidation_shock > 0.0
    ):
        problems.append(
            f"{matrix.arm}: liquidation shock {matrix.liquidation_shock} is not a positive move"
        )
    # A shock above 1.0 is legitimate: it means even a total price collapse of the book would
    # not reach the engine floor, because the account's balance is large relative to exposure.
...
def ruin_summary_rows(matrices: Iterable[WipeoutMatrix]) -> list[dict[str, Any]]:
    ...
    for matrix in matrices:
        rows.append(
            {
                "arm": matrix.arm,
                "declared_twe": matrix.declared_twe,
                "starting_balance_usd": matrix.starting_balance_usd,
                "peak_total_exposure": matrix.peak_total_exposure,
                "peak_exposure_balance_usd": matrix.peak_exposure_balance_usd,
                "liquidation_threshold": matrix.liquidation_threshold,
