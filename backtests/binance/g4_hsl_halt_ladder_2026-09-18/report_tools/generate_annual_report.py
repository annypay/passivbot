#!/usr/bin/env python3
"""Render one arm's deep analysis for the g4 @ TWE 3.0 HSL halt-ladder study.
...
#: This round has no separate synthesis document: the cross-leg comparison lives in the study
#: README, and the pre-registered criteria live in the frozen design contract.
SYNTHESIS_PATH = study.STUDY / "README.md"
DESIGN_PATH = study.STUDY / "halt_ladder_design.md"
...
def scope_lines(
    ...
    leg_roles = {
        "3y": "原生窗口腿（与发布 profile 同数据集/同窗口；这是上一轮参数搜索所用的窗，本轮不搜索）",
        "ext": "全历史腿（同一冻结 bundle 的 dataset override；包含上一轮的搜索窗，"
        "只有 2021-05 崩盘段属于样本外）",
        "pre": "样本外腿（同一冻结 bundle 的 intersection override，窗口在上一轮搜索窗之前，"
        "那一轮的搜索与选臂都看不到它）",
    }
...
def guard_section(ledger: dict[str, Any], analysis: dict[str, Any]) -> list[str]:
    ...
    rows = [
        ...
        [
            "冷却阶梯 `halt_ladder_minutes`",
            (
                f"{[int(value) for value in params.get('halt_ladder_minutes') or []]}"
                f"（第 N 次触发取第 N 档，夹在最后一档）"
                if params.get("halt_ladder_minutes")
                else "关闭 `[]`（停机时长由 `cooldown_minutes_after_red` 决定）"
            ),
        ],
        [
            "阶梯最高触发次数 `hard_stop_ladder_strikes_max`",
            str(int(analysis.get("hard_stop_ladder_strikes_max") or 0)),
        ],
        [
            "累计已实现亏损预算 `realized_loss_budget_pct`",
            f"{float(params.get('realized_loss_budget_pct') or 0.0):.2f}",
        ],
        [
            "触发永久停机的累计已实现回吐 "
            ...
            fmt_metric(
                "hard_stop_realized_loss_halt_pct_max",
                analysis.get("hard_stop_realized_loss_halt_pct_max"),
            ),
        ],
...
def search_section(
    ...
    """This round declares no parameter search; the section states that fact and its precedent."""
    declaration = dict(variant_input.get("no_search") or {})
    provenance = global_metrics.get("search_provenance")
    lines = [
        "### 参数搜索与选择轨迹（本轮不新增搜索）",
        "",
        "- **本轮没有任何参数搜索**：没有搜索契约、没有取值域、没有候选注册表、没有选择文件，"
        "`run.sh` 也没有搜索阶段。四个臂在 `variant_spec.py` 里预注册，"
        "判据在 `halt_ladder_design.md` §7 冻结。",
    ]
    if declaration:
        lines.append(f"- 声明：{declaration.get('reason')}。")
        lines.append(f"- 依据：{declaration.get('precedent')}。")
    if provenance:
        lines.append(f"- 本 arm 携带搜索出身：{provenance}")
    else:
        lines.append(
            f"- 本 arm 不是搜索候选：它是**声明式**臂（`{variant.lever}`），"
            "与第二轮起的同一条重放管线上的臂直接可比。"
        )
    lines.extend(
        [
            "- 因此本报告的每一个读数都是**声明式对照**：没有“更优”意义上的调参结论，"
            "只有“这个键打开后引擎做了什么”的机制结论。",
            "",
        ]
    )
    return lines
...
def out_of_sample_section(
    ...
    if variant.leg == "pre":
        lines.append(
            "- **本腿是唯一的样本外验收窗**：上一轮的参数搜索与候选选择只看到 3y 窗"
            "（2023-09-12 → 2026-09-12），本腿（2021-04-20 → 2023-09-11）在其中不可见。"
            "本轮不搜索，所以本腿的作用是验收“两个新键在这段崩盘历史上是否仍按契约工作”。"
        )
    elif variant.leg == "3y":
        lines.append(
            "- 本腿**就是**搜索窗（上一轮参数搜索所用的窗口）：它的读数对风险几何而言是样本内的，"
            "任何关于档位/预算的结论都必须由 pre 腿（以及 ext 腿的 2021-05 崩盘段）确认。"
        )
    else:
        lines.append(
            "- 本腿覆盖全历史且**包含搜索窗**：只有 2021-05 崩盘段（窗口开头，早于搜索窗起点）"
            "属于样本外；窗口后段与上一轮搜索窗重叠，不能当作独立验证。"
        )
    lines.append(
        f"- 同一条 lever 在三条腿上的并排读数由研究说明给出（`{study.relative(SYNTHESIS_PATH)}` →"
        "“结果”一节）；预注册判据见 `halt_ladder_design.md` §7。本报告只声明本腿的证据等级。"
    )
    lines.append("")
    return lines
...
def verifiable_notes(
    ...
    notes = [
        ...
        + "。",
        f"- 研究级对照：`{study.relative(SYNTHESIS_PATH)}`（四个臂 × 三条腿的并排读数与判据裁决）；"
        f"冻结契约：`{study.relative(DESIGN_PATH)}`（配置键、公式、周期与预注册判据）。",
