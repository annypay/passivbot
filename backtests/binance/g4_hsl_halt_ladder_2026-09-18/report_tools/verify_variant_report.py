#!/usr/bin/env python3
"""Independently verify one arm's deep analysis (HSL halt-ladder study) and its evidence chain.
...
* the frozen config really is the parent config plus the arm's declared changes (including the
  two engine keys this round turns on, at the values the arm declared),
...
REQUIRED_LOCAL = (
    ...
    "risk_geometry.json",
    "annual_analysis.md",
    # `episode_trace.json` is deliberately not required: `trace_episode.py` writes it only for the
    # arm/window a mechanics document cites, and the tracked-evidence check below already treats it
    # as optional. When it is present, `check_episode_trace` re-derives it from the ledger.
)
HOST_PATH_PATTERNS = (
...
def check_arm_identity(
    ...
    # The study's own parent derivation adds the two new engine keys to *both* pside blocks
    # (`variant_spec.PARENT_ENGINE_DEFAULTS`), so they are declared differences from the pinned
    # profile rather than drift. Any other difference is still reported.
    derived_defaults = tuple(path for path, _value in study.PARENT_ENGINE_DEFAULTS)
    for root in study.IDENTITY_ROOTS:
        allowed = (
            *declared_paths.get(root, ()),
            *(path for path in derived_defaults if path.split(".")[0] == root),
        )
        problems.extend(
            f"frozen config {problem}"
            for problem in study.diff_subtrees(
                parent.get(root), expected.get(root), root, allowed=allowed
            )
        )
