#!/usr/bin/env python3
"""Run one arm of the g4 @ TWE 3.0 HSL halt-ladder study offline and write its provenance.
...
* the exposure geometry (limit, slots, allowance, per-slot cap) must be the declared one,
* the account-guard telemetry (`hard_stop_*`) must be recorded for the study's analysis,
* the two engine keys under test (halt ladder, realized-loss budget) must be the declared ones - both of them at their defaults in the `l0_off` regression arm,
...
def declared_geometry(variant: spec.Variant) -> dict[str, Any]:
    """The exposure geometry this arm declares, for the record and the report."""
    ...
    effective = min(allowance, max(0.0, slots - 1.0)) if base > 0.0 else allowance
    return {
        "total_wallet_exposure_limit": twe,
        "n_positions": slots,
        "we_excess_allowance_pct": allowance,
        "we_excess_allowance_mode": spec.PARENT_RISK["we_excess_allowance_mode"],
        "slot_share": base,
...
def search_provenance(variant: spec.Variant) -> None:
    """Always None: this study declares no parameter search, so no arm is search-derived.

    The key is still written to `global_metrics.json` and `run_record.json` as an explicit `None`,
    so the report and the independent verifier state the *absence* of a search instead of leaving
    it implicit (`spec.NO_SEARCH` carries the declaration itself).
    """
    return None
...
def main(argv: list[str] | None = None) -> int:
    ...
    global_metrics = {
        "variant": variant.key,
        "variant_label": spec.lever_for(variant.lever).label,
        ...
        "exposure_caps": {
            **declared_geometry(variant),
            "per_slot_cap": variant.per_slot_cap,
        },
        "search_provenance": search_provenance(variant),
        "legs": list(spec.LEG_ORDER),
        "leg_title": spec.LEG_TITLES.get(variant.leg),
    ...
    run_record = {
        "candidate_id": f"g4_hsl_halt_ladder_{variant.key}",
        "candidate_kind": "hsl_halt_ladder_arm_replay",
        "variant": {
            "key": variant.key,
            "lever": variant.lever,
            "lever_label": spec.lever_for(variant.lever).label,
            "leg": variant.leg,
            ...
            "search_provenance": global_metrics["search_provenance"],
        },
        ...
        "engine_note": (
            ...
            "cost contract. The pinned `a_allow000` reference arms were produced by the previous "
            "engine revision and are this round's control columns; `l0_off` is the bridge that must "
            "reproduce them bit for bit while both new keys sit at their defaults, which is what "
            "makes the two revisions' columns comparable."
        ),
    }
    ...
    print(
        "geometry="
        f"TWE={global_metrics['exposure_caps']['total_wallet_exposure_limit']} "
        f"n_positions={global_metrics['exposure_caps']['n_positions']} "
        f"allowance={global_metrics['exposure_caps']['we_excess_allowance_pct']} "
        f"per_slot_cap={global_metrics['exposure_caps']['per_slot_cap']:.4f}"
    )
    print(
        "halt_ladder="
        f"{variant.declared_halt_ladder_minutes} "
        f"realized_loss_budget_pct={variant.declared_realized_loss_budget_pct} "
        f"no_restart_drawdown_threshold="
        f"{variant.guard_params['no_restart_drawdown_threshold']} "
        f"no_search={global_metrics['search_provenance']}"
    )
