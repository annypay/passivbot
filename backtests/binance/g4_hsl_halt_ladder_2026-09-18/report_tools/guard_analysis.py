...
def declared_guard_from_config(run_dir: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "halt_ladder_minutes": block.get("halt_ladder_minutes"),
        "realized_loss_budget_pct": block.get("realized_loss_budget_pct"),
...
def halt_windows(
    ...
    *,
    ...
    ladder_minutes: Sequence[float] | None = None,
) -> list[HaltWindow]:
    ...
    if ladder_minutes:
        out = _apply_ladder_rungs(out, ladder_minutes)
    return out


def _apply_ladder_rungs(
    halts: Sequence[HaltWindow], ladder_minutes: Sequence[float]
) -> list[HaltWindow]:
    """Re-read every halt's declared length as the ladder rung the engine should have used.

    Mirrors `equity_hard_stop_loss::LadderCycle`: rungs saturate at the last entry, and the cycle
    (so the strike count) restarts when the scope's equity regains the peak the cycle started from.
    The derivation can only sample equity at the flat starts, which is the documented approximation
    here; the engine samples every strategy-equity observation.
    """
    rungs = tuple(float(rung) for rung in ladder_minutes)
    if not rungs:
        return list(halts)
    cycle_peak = 0.0
    strikes = 0
    out: list[HaltWindow] = []
    for halt in halts:
        equity = float(halt.equity_usd)
        if cycle_peak <= 0.0 or equity >= cycle_peak:
            strikes = 0
            cycle_peak = equity
        strikes += 1
        ...
        out.append(
            replace(
                halt,
                declared_minutes=rung,
                shortfall_minutes=max(0.0, rung - halt.minutes),
                excess_minutes=max(0.0, halt.minutes - rung),
            )
        )
    return out
...
def readiness_summary(
    ...
    *,
    ...
    # A per-strike ladder replaces the flat cooldown for the Nth halt of a ladder cycle. Rungs
    # saturate at the last entry (`equity_hard_stop_loss::LadderCycle::halt_minutes`), so the
    # expected length of halt N is `ladder[min(N, len) - 1]`.
    ladder = declared.get("halt_ladder_minutes") or []
    ladder = [float(rung) for rung in ladder]

    def expected_halt_minutes(index: int, *, terminal_halt: bool) -> float | None:
        """Rung the engine should have used for the `index`-th halt (1-based)."""
        if ladder:
            rung = ladder[min(index, len(ladder)) - 1]
            # A terminal halt is the latch, not a cooldown: it lasts until the run ends, so the
            # only meaningful bound is `>= rung`.
            return rung
        if cooldown is None:
            return None
        return cooldown
    ...
    if ladder:
        # With a ladder every halt already carries the rung the cycle rule selected
        # (`_apply_ladder_rungs`, computed from the equity path). Re-deriving it here from the raw
        # halt order would use a different rule and report a false mismatch whenever the cycle
        # resets mid-run, so the ladder case compares the derived duration against that rung (the
        # shortfall check) and its aggregate against the engine's telemetry (below) instead.
        for halt in halts:
            if halt.declared_minutes is None:
                problems.append(
                    f"halt starting {halt.start.isoformat()} has no ladder rung to compare "
                    "against"
                )
    ...
    if triggers:
        if ladder:
            expected_total = sum(
                expected_halt_minutes(index, terminal_halt=False) or 0.0
                for index in range(1, len(complete) + 1)
            )
            longest_expected = max(
                (expected_halt_minutes(index, terminal_halt=False) or 0.0
                 for index in range(1, len(halts) + 1)),
                default=0.0,
            )
        else:
            expected_total = cooldown * len(complete) if cooldown else 0.0
            longest_expected = cooldown if cooldown is not None else 0.0
        expected_total += sum(halt.minutes for halt in terminal)
        if (
            (cooldown is not None or ladder)
            and not terminal
            and max_minutes > longest_expected + tolerance_minutes
        ):
            problems.append(
                f"the engine reports a longest halt of {max_minutes:.0f} min but no terminal "
                f"halt was derived and the declared cooldown/ladder allows only "
                f"{longest_expected:.0f} min"
            )
...
def build_guard_artifact(
    ...
    halts = halt_windows(
        ...
        ladder_minutes=declared.get("halt_ladder_minutes") or None,
    )
