#!/usr/bin/env python3
"""Run one arm of the g4 tail-risk study offline and write this study's provenance.
...
def require_preconditions(variant: spec.Variant, *, force: bool, reuse_run: bool) -> None:
    ...
    if variant.synthetic:
        if not spec.SYNTHETIC_BUNDLES_PATH.exists():
            problems.append(
                f"missing {spec.relative(spec.SYNTHETIC_BUNDLES_PATH)}; run "
                "make_synthetic_collapse_bundle.py first"
            )
        elif not dataset.path.is_dir():
            problems.append(
                f"missing synthetic bundle {dataset.rel_path}; run "
                "make_synthetic_collapse_bundle.py first"
            )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if variant.synthetic and spec.SYNTHETIC_BUNDLES_PATH.exists():
        synthetic = spec.load_json(spec.SYNTHETIC_BUNDLES_PATH)
        dataset_identity["synthetic_record"] = synthetic.get("bundles", {}).get(variant.dataset.key)
    ...
    run_record = {
        "candidate_id": f"g4_tail_risk_{variant.key}",
        "candidate_kind": "tail_risk_arm_replay",
        "variant": {
            "key": variant.key,
            "lever": variant.lever,
