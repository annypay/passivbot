#!/usr/bin/env python3
"""Render one arm's deep analysis for the g4 tail-risk study.
...
def ledger_facts(
    ...
    reference = study.REFERENCE_RUNS["ref_hsl_off_3y"] if variant.leg == "3y" else None
    return {
        "variant": variant.key,
...
def synthetic_record(variant: study.Variant) -> dict[str, Any] | None:
    if not variant.synthetic or not study.SYNTHETIC_BUNDLES_PATH.exists():
        return None
    return (study.load_json(study.SYNTHETIC_BUNDLES_PATH).get("bundles") or {}).get(
        variant.dataset.key
    )
...
def build_tail_artifacts(
    ...
    record = synthetic_record(variant)
        ...
        "synthetic_injections": [
            {
                "key": episode.key,
                "label": episode.label,
                "coin": episode.label.split("：")[1].split(" ")[0] if "：" in episode.label else None,
                "peak_ms": episode.peak_ms,
                "trough_ms": episode.trough_ms,
                "drop_pct": episode.drop_pct,
            }
            for episode in injections
        ],
        "full_window": {
    ...
    return table, events_payload, wipeout_payload, variant_risk_block(result_dir)
...
def scope_lines(
    ...
    if variant.synthetic:
        lines.append(
            "- **合成情景声明**：本 arm 使用的数据集是原生 bundle 的派生副本，其中"
            "目标币的价格路径被注入为崩塌路径（见 `report_tools/make_synthetic_collapse_bundle.py`）。"
            "它是反事实压力测试，不是历史事实，也不能代表真实退市流程。"
        )
    lines.append(
...
def wipeout_section(wipeout_payload: dict[str, Any]) -> list[str]:
    ...
    lines = [
        "### 暴露上界与爆仓距离（单币/多币归零）",
        "",
        f"- 实测总暴露峰值 `total_wallet_exposure_max` = {ruin['peak_total_exposure']:.6f}，"
        f"对应爆仓距离（整篮在该峰值处归零后剩余权益比例）= **{ruin['ruin_distance'] * 100:.2f}%**。",
...
def live_reading_section(
    ...
    lines.append(
        ...
        f"整篮在实测峰值处归零 ≈ −{(1 - ruin['ruin_distance']) * 100:.2f}%；这意味着"
        "“一个币归零带走全部本金”在引擎层面不成立，真正的尾部来自多币同时崩塌与恢复基数被打掉。"
    )
    if ledger["hsl_block"].get("enabled"):
        ...
        lines.append("- 未启用任何账户级熔断：若要在事件内部切断损失链，需要显式启用 HSL 账户级作用域。")
    lines.append("")
    return lines
...
def verifiable_notes(
    ...
    notes = [
        ...
        "（该脚本不 import 本渲染器，重算事件表、冲击矩阵与账本数字）。",
    ]
    if ledger["synthetic"]:
        notes.append(
            f"- 合成数据集记录：`{study.relative(study.SYNTHETIC_BUNDLES_PATH)}`"
            "（含注入参数、目标币与派生 bundle 的哈希）。"
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.skip_tail_artifacts:
        ...
        event_rows, events_payload, wipeout_payload, _risk = build_tail_artifacts(result_dir, variant)
    ...
    context: dict[str, Any] = {
        ...
        "interpretation_extra": [
            ...
            f"尾部读数的口径是：事件表按基准驱动的事件窗口测量，冲击矩阵按实测敞口峰值给出上界，"
            f"两者都不含对未来的推断。",
        ],
    }
