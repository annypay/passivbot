#!/usr/bin/env python3
"""Render one arm's deep analysis for the g4 @ TWE 3.0 risk-geometry study.
...
GEOMETRY_ARTIFACT = study.GEOMETRY_ARTIFACT
...
def ledger_facts(
    ...
    reference_key = study.REFERENCE_CONTROL_BY_LEG.get(variant.leg)
    reference = study.REFERENCE_RUNS.get(reference_key) if reference_key else None
    if reference is None:
        reference = next(
            (
                item
                for item in study.references_of_kind("no_guard").values()
                if item.get("leg") == variant.leg
            ),
            None,
        )
...
def geometry_artifact(result_dir: Path) -> dict[str, Any]:
    return study.load_json(result_dir / GEOMETRY_ARTIFACT)
...
def build_tail_artifacts(
    ...
    geometry_payload = geometry.build_geometry_artifact(result_dir, analysis)
    geometry_payload["arm"] = variant.key
    geometry_payload["leg"] = variant.leg
    geometry_payload["stage"] = study.stage_of(variant)
    geometry_payload["declared_twe"] = variant.declared_twe
    geometry_payload["declared_n_positions"] = variant.declared_n_positions
    geometry_payload["declared_allowance_pct"] = variant.declared_allowance_pct
    geometry_payload["per_slot_cap"] = variant.per_slot_cap
    if geometry_payload["cross_check_problems"]:
        fail(
            [f"geometry: {problem}" for problem in geometry_payload["cross_check_problems"]],
            f"{variant.key}: the exposure geometry violates its own invariants",
        )
    study.write_json(result_dir / GEOMETRY_ARTIFACT, geometry_payload)
    return (
        table,
        events_payload,
        wipeout_payload,
...
def scope_lines(
    ...
    leg_roles = {
        "3y": "原生搜索窗腿（与发布 profile 同数据集/同窗口；参数搜索只看到这条腿）",
        "ext": "全历史腿（同一冻结 bundle 的 dataset override；包含搜索窗，"
        "只有 2021-05 崩盘段属于样本外）",
        "pre": "样本外腿（同一冻结 bundle 的 intersection override，窗口在搜索窗之前，"
        "搜索与选臂都看不到它）",
    }
    leg_note = leg_roles.get(variant.leg, f"未知腿 {variant.leg!r}")
...
def _fmt_optional(value: Any, *, scale: float = 1.0, digits: int = 2, suffix: str = "") -> str:
    if value is None:
        return "n/a"
    try:
...
def geometry_section(geometry_payload: dict[str, Any]) -> list[str]:
    """Exposure geometry and occupancy discipline: the round's first question."""
    declared = geometry_payload.get("declared") or {}
    observed = geometry_payload.get("observed") or {}
    excess = observed.get("per_slot_cap_excess_pct")
    rows = [
        ["总暴露上限 `total_wallet_exposure_limit`", f"{declared.get('total_wallet_exposure_limit', 0):.2f}"],
        ["槽位 `n_positions`", f"{declared.get('n_positions', 0):.0f}"],
        [
            "占用余量 `we_excess_allowance_pct`（原始 / 生效）",
            f"{declared.get('we_excess_allowance_pct', 0):.2f} / "
            f"{declared.get('effective_allowance_pct', 0):.2f}"
            f"（`{declared.get('we_excess_allowance_mode')}` 模式）",
        ],
        ["均分额度 `TWE / n_positions`", f"{declared.get('slot_share', 0):.4f}"],
        ["单槽敞口上限", f"{declared.get('per_slot_cap', 0):.4f}"],
        [
            "峰值单币敞口（实测）",
            f"{observed.get('peak_coin_exposure', 0):.4f}"
            + (f"（超出上限 {excess * 100:+.3f}%）" if excess is not None else ""),
        ],
        [
            "峰值单币 / 峰值总暴露",
            _fmt_optional(observed.get("peak_coin_share_of_peak_total"), scale=100, suffix="%"),
        ],
        ["前 3 币峰值之和", f"{observed.get('top3_exposure', 0):.4f}"],
        [
            "总暴露（峰值 / 均值）",
            f"{observed.get('peak_total_exposure', 0):.4f} / "
            + _fmt_optional(observed.get("mean_total_exposure"), digits=4),
        ],
        [
            "在场币数（时间加权均值 / 峰值）",
            _fmt_optional(observed.get("active_coins_mean"))
            + " / "
            + _fmt_optional(observed.get("active_coins_peak"), digits=0),
        ],
        [
            "空槽时间占比（按均值口径）",
            _fmt_optional(observed.get("empty_slot_time_share"), scale=100, suffix="%"),
        ],
        [
            "有仓位时间占比",
            _fmt_optional(observed.get("time_with_any_position_share"), scale=100, suffix="%"),
        ],
        [
            "单币完全归零的损失上界",
            f"{observed.get('single_coin_wipeout_bound', 0) * 100:.2f}% 账户",
        ],
        [
            "峰值暴露处到强平地板的跌幅",
            _fmt_optional(observed.get("liquidation_shock_at_peak"), scale=100, suffix="%"),
        ],
    ]
    return [
        "### 暴露几何与占用纪律",
        ...
        spec.md_table(rows, ["项目", "数值"]),
        "",
        f"- 推导口径：{geometry_payload.get('method', 'n/a')}",
        "- 读法：**单槽上限是“下单规划时”的约束**（引擎按当时的余额计算），成交账本记录的是成交时的"
        "`wallet_exposure`，两者之间有盯市漂移与交易所最小下单量的差，因此本表同时给出实测超出比例；"
        f"几何不变量（单币敞口 ≤ 上限 × {1 + geometry.PER_SLOT_CAP_TOLERANCE_PCT:.2f}"
        f" + {geometry.PER_SLOT_CAP_ABSOLUTE_SLACK:.2f}、总量 ≤ 总暴露上限、"
        "在场币数 ≤ `n_positions`）在生成与复核时都校验；本轮实测的最大绝对超出是 0.0191"
        "（最紧的上限 TWE 2.5/7 槽），因此绝对余量按 0.02 声明。",
        "- “空槽时间占比”按时间加权的在场币数折算：它衡量的是**未把额度用满的时间比例**，"
        "与“是否持仓”是两个不同读数（后者见“有仓位时间占比”）。",
        "",
    ]


def search_section(
    variant: study.Variant, global_metrics: dict[str, Any], variant_input: dict[str, Any]
) -> list[str]:
    """The parameter-search contract, and for a search-derived arm its frozen selection entry."""
    contract = dict(variant_input.get("search_contract") or {})
    selection_payload = variant_input.get("search_selection") or {}
    recorded_search = selection_payload.get("search") or {}
    # The frozen contract declares the plan; the recorded block is what actually ran (the smoke
    # test may have lowered `n_cpus` or the budget), so the report cites the recorded values and
    # falls back to the contract before the search has run.
    for key in ("seed", "iters", "population_size", "n_cpus"):
        if recorded_search.get(key) is not None:
            contract[key] = recorded_search[key]
    provenance = global_metrics.get("search_provenance")
    bounds = contract.get("bound_leaves") or {}
    rows = [
        [
            f"`{key}`",
            f"[{bound[0]:g}, {bound[1]:g}] 步长 {bound[2]:g}",
        ]
    ...
    lines = [
        ...
        if rows
        else "- 本 arm 生成时搜索契约尚未冻结。",
        "",
        f"- 搜索实现：仓库既有 pymoo 优化器（`optimize.backend=pymoo`），搜索腿 "
        f"`{contract.get('candidate_legs', ['3y'])[0]}`，种子 {contract.get('seed')}、"
        f"`iters`={contract.get('iters')}、`population_size`={contract.get('population_size')}、"
        f"`n_cpus`={contract.get('n_cpus')}；目标函数 "
        + "；".join(
            f"{item['metric']}（{item['goal']}）" for item in (contract.get("scoring") or [])
        )
        + "。",
        f"- 参考约束（`optimize.limits`）："
        + "；".join(
            f"{item['metric']} {item['penalize_if']} {item['value']}"
            for item in (contract.get("limits") or [])
        )
        + "。",
        f"- 选择规则（看到样本外结果之前执行）：{contract.get('pick')}；候选腿 "
        f"{contract.get('candidate_legs')}。",
        "- 钉死不变（`optimize.fixed_params`）："
        + "、".join(f"`{key}`" for key in (contract.get("fixed_params") or [])[:6])
        + f" 等共 {len(contract.get('fixed_params') or [])} 个 alpha 键；"
        "`live.hsl_signal_mode`/`live.pnls_max_lookback_days`/`live.max_realized_loss_pct`"
        "与短边全部键也不在搜索域内。",
    ...
    if selection:
        lines.append(
            f"- 冻结的选择文件：`{selection.get('path')}`（sha256 `{str(selection.get('sha256'))[:16]}…`，"
            f"候选 {len(selection.get('selected') or [])} 个）。"
        )
    if provenance:
        ...
        lines.extend(
            [
                f"- **本 arm 来自搜索候选**（{provenance.get('selection_kind')}）："
                + "、".join(f"`{key}`={value}" for key, value in sorted(params.items()))
                + "。",
                f"- 候选选择时的样本内读数："
                + "、".join(f"{key}={value}" for key, value in sorted(metrics.items()))
                + "。",
                f"- 证据来源：`{provenance.get('pareto_path')}`"
                f"（sha256 `{str(provenance.get('pareto_sha256'))[:16]}…`，"
                f"存在={provenance.get('pareto_present')}）。",
            ]
        )
    else:
        lines.append(
            "- 本 arm 不是搜索候选：它是**声明式**臂（占用纪律/档位网格），"
            "与搜索候选在同一条管线上重放，因此两者可以直接比较。"
        )
    lines.append("")
    return lines


def out_of_sample_section(
    variant: study.Variant, variant_input: dict[str, Any], analysis: dict[str, Any]
) -> list[str]:
    """What this leg can and cannot prove, and this arm's own reading on it."""
    legs = {item["key"]: item for item in (variant_input.get("legs") or [])}
    leg = legs.get(variant.leg, {})
    lines = [
        "### 样本外读数（pre 腿）",
        "",
        f"- 本 arm 所在腿：**{leg.get('title', variant.leg)}**；窗口 "
        f"{(leg.get('window') or ['?', '?'])[0]} → {(leg.get('window') or ['?', '?'])[1]}；"
        f"角色：{leg.get('role', 'n/a')}。",
        f"- 本 arm 在该腿上的读数：终值 {_fmt_optional(analysis.get('gain_strategy_eq'), digits=4)}×、"
        f"最差回撤 {_fmt_optional(analysis.get('drawdown_worst_strategy_eq'), scale=100, suffix='%')}、"
        f"强平={bool(analysis.get('liquidated'))}。",
    ]
    if variant.leg == "pre":
        lines.append(
            "- **本腿是唯一的样本外验收窗**：参数搜索与候选选择只看到 3y 窗"
            "（2023-09-12 → 2026-09-12），本腿（2021-04-20 → 2023-09-11）在其中都不可见，"
            "所以本腿上的生存/回撤读数是搜索结论的样本外证据。"
        )
    elif variant.leg == "3y":
        lines.append(
            "- 本腿**就是**搜索窗：任何“更优”都只能算样本内证据，必须由 pre 腿（以及 ext 腿的"
            "2021-05 崩盘段）确认；报告的综合结论按这个优先级读。"
        )
    else:
        lines.append(
            "- 本腿覆盖全历史且**包含搜索窗**：只有 2021-05 崩盘段（窗口开头，早于搜索窗起点）"
            "属于样本外；窗口后段与搜索窗重叠，不能当作独立验证。"
        )
    lines.append(
        f"- 同一条 lever 在三条腿上的并排读数由综合文档给出（`{study.relative(SYNTHESIS_PATH)}` →"
        "“样本外与跨腿对照”）；本报告只声明本腿的证据等级。"
    )
    lines.append("")
    return lines
...
def verifiable_notes(
    ...
    notes = [
        ...
        f"- 暴露几何：TWE {facts['risk']['total_wallet_exposure_limit']} / "
        f"`n_positions` {facts['risk']['n_positions']} / "
        f"`we_excess_allowance_pct` {facts['risk']['we_excess_allowance_pct']} ⇒ 单槽上限 "
        f"{facts['ledger']['single_slot_exposure_cap']:.4f}；占用读数见“暴露几何与占用纪律”。",
    ]
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.skip_tail_artifacts:
        ...
        geometry_payload = geometry_artifact(result_dir)
    else:
        (
            event_rows,
            events_payload,
    ...
    appendix_parts = [
        ...
        *geometry_section(geometry_payload),
        *search_section(variant, global_metrics, variant_input),
        *out_of_sample_section(variant, variant_input, analysis),
    ...
    context: dict[str, Any] = {
        ...
        "interpretation_extra": [
            f"本 arm 的杠杆是 `{variant.lever}`（{study.lever_for(variant.lever).label}）："
            f"{study.lever_for(variant.lever).hypothesis}",
