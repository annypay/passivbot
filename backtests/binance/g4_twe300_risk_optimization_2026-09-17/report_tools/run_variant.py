#!/usr/bin/env python3
"""Run one arm of the g4 @ TWE 3.0 risk-geometry study offline and write its provenance.
...
* a search-derived arm must cite the frozen selection entry it came from,
...
def search_provenance(variant: spec.Variant) -> dict[str, Any] | None:
    """For a search-derived arm: the frozen selection entry plus the pareto file it came from."""
    selection = spec.load_search_selection()
    if not selection:
        return None
    for candidate in selection.get("selected") or []:
        if candidate.get("lever") != variant.lever:
            continue
        entry = {
            "selection_path": spec.relative(spec.SEARCH_SELECTION_PATH),
            "selection_sha256": spec.sha256_file(spec.SEARCH_SELECTION_PATH),
            "lever": candidate.get("lever"),
            "selection_kind": candidate.get("selection_kind"),
            "parameters": candidate.get("parameters"),
            "metrics": candidate.get("metrics"),
            "picker": selection.get("search", {}).get("pick"),
        }
        pareto_path = candidate.get("pareto_path")
        if pareto_path:
            resolved = Path(pareto_path)
            entry["pareto_path"] = spec.relative(resolved)
            entry["pareto_sha256"] = (
                spec.sha256_file(resolved) if resolved.exists() else None
            )
            entry["pareto_present"] = resolved.exists()
        return entry
    return None
...
def main(argv: list[str] | None = None) -> int:
    ...
    if global_metrics["search_provenance"]:
        print(
            "search: lever="
            f"{global_metrics['search_provenance']['lever']} "
            f"kind={global_metrics['search_provenance']['selection_kind']} "
            f"pareto_present={global_metrics['search_provenance'].get('pareto_present')}"
        )
