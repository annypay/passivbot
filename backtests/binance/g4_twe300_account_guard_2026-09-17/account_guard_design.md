...
> 2026-09-17 更新（Round D 已完成）：分级近似（软刹车）已被证伪；固定档位对照表明加长冷却不更优；
> 占用纪律在"守护已开启"时同时牺牲终值与回撤（详见 `g4_twe300_risk_optimization_2026-09-17`）。
> 因此 §6.3 的定档结论生效：引擎级阶梯按"风险塑形、默认关闭、最高档 24H"设计。
...
### 6.3 冷却档位的定档结论（Round D 实测，2026-09-17）

`g4_twe300_risk_optimization_2026-09-17` 把固定档位 12/24/48/72H 放在同一条腿上重放
（allowance=0 + unified 0.20 / EMA 60），裁决 J3 的结果是：

| 档位 | ext 腿终值 / 最差回撤 | pre 腿（样本外）终值 / 最差回撤 |
|---|---|---|
| 12H | 2.3002× / 76.56% | 0.6104× / 76.56% |
| 24H | 2.7662× / 76.85% | 0.5888× / 76.85% |
| 48H | 2.0828× / 77.10% | 0.5554× / 77.10% |
| 72H | 1.8666× / 79.29% | 0.4832× / 79.29% |

**结论：加长冷却在两条腿上都不更优**（档位越高，回撤单调变差，终值在 pre 腿上单调变差）。
因此下一轮的引擎级阶梯应当：

1. 把最高档设在 **24H**，而不是 48/72H——`halt_ladder_minutes` 的默认建议写成 `[720, 1440]`；
2. 在文档与 CHANGELOG 里明确写：**阶梯是风险塑形，不是收益改进**（本轮与上一轮的证据都指向这一点），
   它的价值是"在连续触发时少复入几次"，而不是"赚得更多"；
3. 因此阶梯必须默认关闭，并由使用者在理解代价后显式开启。

同轮实测还给了两条与本设计相关的硬约束：

- **累计锁存阈值必须高于存活臂的最差回撤**：`no_restart_drawdown_threshold` 取 0.55 时在 ext 腿
  **首击锁存**（终值 0.3399×），取 0.70 时第二次触发锁存（0.2910×），而同一几何下不锁存的臂
  最差回撤是 76.56% —— 也就是说，任何低于 ~0.77 的永久地板都会在这段历史里把账户关停。
  永久停机应改用**累计已实现亏损**口径（本设计的 `realized_loss_budget_pct`），而不是瞬时回撤尖峰。
- **实现亏损刹车在本配置下没有可测效果**：`live.max_realized_loss_pct=0.50` 的 ext 腿读数与
  未开启时逐位相同（2.3002× / 76.56%），说明它没有约束到任何一笔平仓；它可以作为一条可选防线，
  但不能当作已验证的保护。

### 6.4 复牌时点：新出现的、值得单独实验的缺口
