...
| `binance/g4_hsl_halt_ladder_2026-09-18` | 引擎级冷却阶梯与累计已实现亏损口径的声明式验证：四个预注册臂（默认关闭 / `[720,1440]` / `realized_loss_budget_pct=0.30` / 固定 24H）× 三条腿，证明默认关闭时与改动前逐位一致（`ext` 腿 45,644 笔成交哈希相同、该腿守护触发 4 次）、阶梯档位按第 N 次触发递进，同时给出「阶梯收益中性、0.30 预算会在第一次停机就永久退出」与「第一档取舍在两条腿上相反」的实测；中文 README 与设计冻结件 `halt_ladder_design.md` 见该 study 目录。 |
| `binance/g4_overfitting_audit_2026-09-18` | 跨轮次过拟合审计：trial ledger（N 下界 1805）、两个池的三腿月度面板、CSCV/PBO（S∈{8,10,12,16} × 两种选择口径 + 预注册退化剔除）、DSR/MinBTL/平稳自助 SPA 与 StepM（N 敏感性 92/200/500/1807）、K=3/6 折叠稳定性、45 个声明式成本压力臂、R1–R22 反模式复核；J1/J3/J4 样本外不通过 ⇒ J5 不冻结。中文 README 与 `overfitting_audit.md`（含 §13 裁决）见该 study 目录。 |
...
# TWE 3.0 account guard: 12 arms (six guard shapes over two legs) around the requested
# "-20% in a week halts for 12h" breaker, with per-halt resume readings and a synthesis.
